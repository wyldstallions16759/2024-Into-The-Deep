...
@Autonomous(name="28147 Two SPEC")

//@Disabled

public class Auto28147AdvancedDouble extends LinearOpMode {
    ...
    static final Pose2D SUBMERSIBLE3 = new Pose2D(DistanceUnit.INCH, -28, -20, AngleUnit.DEGREES, 0);
    static final int ARM_ELEV_PLACE_SPECIMEN = -2000;
    static final int ARM_ELEV_PICK_SAMPLE = -5866;
...
//                }
            ...
            //----------------------------------------------------------
            ...
            else if (stateMachine == StateMachine.PICK_SAMPLE) {
                arm.setElevationTarget(ARM_ELEV_PICK_SAMPLE);
                boolean armTo = arm.armUp(ARM_ELEVATION_POWER);
                if (armTo) {
                    stateMachine = StateMachine.CLAMP;
                }
            }
                ...
                // Move to next state only when all three operations complete
                wrist.wristUp();
                boolean driveTargetReached = pinpoint.driveTo(SUBMERSIBLE3, DRIVE_SPEED, 0);
//                if (driveTargetReached) {
//                    stateMachine = StateMachine.DRIVE_TO_GP_1A;
//                }

                arm.setElevationTarget(ARM_ELEV_PLACE_SPECIMEN-1000);
                boolean armElevReached = arm.armDown(ARM_ELEVATION_POWER);

                arm.setExtensionTarget(ARM_EXTEND_PLACE_SPECIMEN+2000);
                boolean armExtReached = arm.armExtend(ARM_EXTENSION_POWER);

                // If all three conditions are met, move to next state to retract the arm
                if (driveTargetReached && armElevReached && armExtReached) {
                    stateMachine = StateMachine.ARM_UP_1_B_AGAIN;
                }
            }
            else if (stateMachine == StateMachine.ARM_UP_1_B_AGAIN) {
                // In parallel:
                // (a) drive to front of submersible
                // (b) rotate arm to the specified position
                // (c) extend arm to the specified position
