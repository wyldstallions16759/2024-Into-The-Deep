...
public class DriveToPoint {
    ...
    private double getHeadingErrorInDegrees(Pose2D targetPose, Pose2D currPose) {
        double currHeading = currPose.getHeading(DEGREES);
        double targetHeading = targetPose.getHeading(DEGREES);
        ...
        return Math.min(Math.abs(targetHeading - currHeading), 360 - Math.abs(targetHeading - currHeading));
    }

    private boolean inBounds(Pose2D currPose, Pose2D trgtPose) {
        xOutOfBounds = currPose.getX(MM) > (trgtPose.getX(MM) - xyTolerance) && currPose.getX(MM) < (trgtPose.getX(MM) + xyTolerance);
        yOutOfBounds = currPose.getY(MM) > (trgtPose.getY(MM) - xyTolerance) && currPose.getY(MM) < (trgtPose.getY(MM) + xyTolerance);

        boolean hOutOfBounds = getHeadingErrorInDegrees(trgtPose, currPose) <= RADIANS.toDegrees(yawTolerance);
//        boolean hOutOfBounds = currPose.getHeading(RADIANS) > (trgtPose.getHeading(RADIANS) - yawTolerance) &&
//                currPose.getHeading(RADIANS) < (trgtPose.getHeading(RADIANS) + yawTolerance);
    ...
    public double getYawTolerance() {
        return RADIANS.toDegrees(yawTolerance);
    }
